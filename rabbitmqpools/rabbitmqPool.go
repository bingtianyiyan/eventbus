...
type ConsumeReceive struct {
	TagName      string                                                           //标识处理方法
	ExchangeName string                                                           //交换机
	ExchangeType string                                                           //交换机类型
	Route        string                                                           //路由
	QueueName    string                                                           //队列名称
	EventSuccess func(data *amqp.Delivery, retryClient RetryClientInterface) bool //成功事件回调
...
func (r *RabbitPool) ConnectUrl(url string) error {
	r.url = url
	return r.initConnectionsWithUrl(false)
}
...
func (r *RabbitPool) DeclareExchange(rconn *rConn, channel *rChannel, exChangeName, exChangeType string, args amqp.Table) error {
	...
	if err != nil {
		log(fmt.Sprintf("ExchangeDeclarePassive->exChangeName:%s,exChangeType:%s", exChangeName, exChangeType))
		//重连 生成处理channel 根据最大channel数处理
		newChannel, err = rCreateChannel(rconn)
		if err != nil {
			return errors.New(fmt.Sprintf("channel-exchange重建失败:%s", err))
		}
		err = newChannel.ExchangeDeclare(exChangeName, exChangeType, true, false, false, false, nil)
		if err != nil {
			return errors.New(fmt.Sprintf("MQ注册交换机失败:%s", err))
		}
	}
...
func (r *RabbitPool) DeclareQueue(rconn *rConn, channel *rChannel, queueName string, args amqp.Table) (string, error) {
	newChannel := channel.ch
	queue, err := newChannel.QueueDeclarePassive(queueName, true, false, false, false, args)
	if err != nil {
		log(fmt.Sprintf("QueueDeclarePassive->queueName:%s", queueName))
		//重连 生成处理channel 根据最大channel数处理
		newChannel, err = rCreateChannel(rconn)
		if err != nil {
			return "", errors.New(fmt.Sprintf("channel-queue重建失败:%s", err))
		}
		queue, err = newChannel.QueueDeclare(queueName, true, false, false, false, args)
		if err != nil {
...
func (r *RabbitPool) BindQueue(rconn *rConn, channel *rChannel, queueName, routingKey, exchange string, args amqp.Table) error {
	...
	if err != nil {
		return errors.New(fmt.Sprintf("绑定队列%s到路由：%s,交换机:%s失败:%s", queueName, routingKey, exchange, err))
	}
	return nil
}
...
func (r *RabbitPool) CreateRChannel(channel *amqp.Channel, num int32) *rChannel {
	return &rChannel{ch: channel, index: num}
}
...
初始化连接池
*/
func (r *RabbitPool) initConnectionsWithUrl(isLock bool) error {
	r.connections[r.clientType] = []*rConn{}
	var i int32 = 0
	for i = 0; i < r.maxConnection; i++ {
		itemConnection, err := rConnectWithUrl(r, isLock)
		if err != nil {
			return err
		} else {
			r.connections[r.clientType] = append(r.connections[r.clientType], &rConn{conn: itemConnection, index: i})
...
func rConnect(r *RabbitPool, islock bool) (*amqp.Connection, error) {
	if len(r.url) > 0 { //优先url
		return rConnectWithUrl(r, islock)
	}
	virtualHost := "/"
	if len(strings.TrimSpace(r.virtualHost)) > 0 {
		virtualHost = r.virtualHost
	}
	return connection(r.user, r.password, r.host, r.port, virtualHost, r.url)
}
...
原rabbitmq连接
*/
func rConnectWithUrl(r *RabbitPool, islock bool) (*amqp.Connection, error) {
	return connectionWithUrl(r.url)
}

func connection(user string, password string, host string, port int, vHost string, url string) (*amqp.Connection, error) {
	if len(url) > 0 {
		return connectionWithUrl(url)
	}
	virtualHost := "/"
...
func connectionWithUrl(url string) (*amqp.Connection, error) {
...
func rDeclare(rconn *rConn, clientType int, channel *rChannel, exChangeName string, exChangeType string, queueName string, route string, isDeadQueue bool, oldExChangeName string, oldQueueName, oldRoute string) (*rChannel, error) {
	...
	if err != nil {
		...
		newChannel, err = rCreateChannel(rconn)
		if err != nil {
			return nil, errors.New(fmt.Sprintf("channel-exchange重建失败:%s", err))
		}
		err = newChannel.ExchangeDeclare(exChangeName, exChangeType, true, false, false, false, nil)
		if err != nil {
	...
	if (clientType == RABBITMQ_TYPE_PUBLISH && exChangeType != EXCHANGE_TYPE_FANOUT) || (clientType == RABBITMQ_TYPE_CONSUME && (exChangeType == EXCHANGE_TYPE_FANOUT || exChangeType == EXCHANGE_TYPE_DIRECT)) {
		...
		queue, err := newChannel.QueueDeclarePassive(queueName, true, false, false, false, argsQue)
		if err != nil {
			log(fmt.Sprintf("QueueDeclarePassive->queueName:%s", queueName))
			//重连 生成处理channel 根据最大channel数处理
			newChannel, err = rCreateChannel(rconn)
			if err != nil {
				return nil, errors.New(fmt.Sprintf("channel-queue重建失败:%s", err))
			}
...
func consumeTask(num int32, pool *RabbitPool, receive *ConsumeReceive) {
	...
	//rChannels, err = rDeclare(conn, pool.clientType, rChannels, receive.ExchangeName, receive.ExchangeType, receive.QueueName, receive.Route, receive.IsDead, receive.DeadExchangeName, receive.DeadQueueName, receive.DeadRoute)
	rChannels, err = rDeclare(conn, pool.clientType, rChannels, receive.ExchangeName, receive.ExchangeType, receive.QueueName, receive.Route, false, "", "", "")
	// 重新赋值channel,在rDeclare里可能会关闭channel
	channel = rChannels.ch
	//如果存在死信队列 则需要声明
	if receive.IsTry {
		...
		if num%2 == 0 {
			...
			deadrChannels, err = rDeclare(conn, pool.clientType, deadrChannels, deadExchangeName, EXCHANGE_TYPE_DIRECT, deadQueueName, deadRouteKey, true, receive.ExchangeName, receive.QueueName, receive.Route)
			// 重新赋值channel,在rDeclare里可能会关闭channel
			deadChannel = deadrChannels.ch
			//channel = deadrChannels.ch
		}
	}
	if err != nil {
		if receive.EventFail != nil {
	...
	////判断channel是否关闭
	//isClose := isChannelClosed(channel)
	//if isClose {
	//	channel, err = rCreateChannel(conn)
	//	if err != nil {
	//		if receive.EventFail != nil {
	...
	for {
		...
		case data := <-msgs:
			...
			if receive.EventSuccess != nil {
				...
				isOk := receive.EventSuccess(&data, retryClient)
...
// 重连
func reconnect(pool *RabbitPool, rc *rConn, channel *amqp.Channel, isRetry bool) error {
	if isRetry {
		log("连接中断,2秒后开始重试-重连")
		time.Sleep(time.Second * 2)
	}
	...
	if (rc.conn == nil || rc.conn.IsClosed()) || isChannelClosed(channel) {
		rc.conn, err = connection(pool.user, pool.password, pool.host, pool.port, pool.virtualHost, pool.url)
		if err != nil {
			err = reconnect(pool, rc, channel, true)
		} else {
			channel, err = rc.conn.Channel()
			if err != nil {
				//logger.Logger.Error("打开channel失败", zap.Error(err))
				return err
			}
		}
	}
...
// 判断通道是否关闭
func isChannelClosed(channel *amqp.Channel) bool {
	closeChan := make(chan *amqp.Error, 1)
	// 注册通道关闭的监听器
